...
def get_stylized_hypergraph(H):
    ...
    for node in H.nodes:
        stylized_hypergraph["nodes"].append({"id" : node, "x_canvas": node_pos[node][0], "y_canvas" : node_pos[node][1], "radius" : 1.7, "color" : "#79aaa0"})
    for edge in H.edges:
        stylized_hypergraph["hyperedges"].append({"id" : edge, "x_canvas": edge_pos[edge][0], "y_canvas" : node_pos[edge][1], "radius" : 1.7, "color" : "#79aaa0"})
    
    for node in H.nodes:
        for edge in H.nodes.memberships(node):
            stylized_hypergraph["bipartite_edges"].append({"source" : node, "target" : edge, "width" : 1.6, "color" : "#7c7c7c"})
