...
def draw_network(network, node_positions):
    ...
    for node in network:
        ...
        net_vis.append(shapes.Circle(x=x, y=y, radius=1, color=(255, 0, 255), batch=batch))
...
def draw_hypergraph(H, node_positions, edge_positions):
    ...
    for edge_id in H.edges:
        ...
        vis.append(shapes.Rectangle(x=x, y=y, width=10, height=10, color=(0, 255, 0), batch=batch))
    ...
    for edge_id in H.edges:
        ...
        vis.append(shapes.Line(x1, y1, x2, y2, color=(255, 255, 255), batch=batch, width=5))
