...
class App(pyglet.window.Window):
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.BACKSPACE or (
            symbol == key._0 and (modifiers & key.MOD_COMMAND)
        ):
...
def get_hypergraph_batch(
    ...
    hyperedge_pos = {
        edge["id"]: np.array([edge["x_canvas"], edge["y_canvas"] + yoffset])
        for edge in stylized_hypergraph["hyperedges"]
    }
    ...
    if draw_nodes:
        ...
        for node in stylized_hypergraph["nodes"]:
            ...
            circles[node["id"]] = shapes.Arc(
                node["x_canvas"],
                node["y_canvas"] + yoffset,
                node["radius"],
                segments=n_circle_segments + 1,
                color=tuple(
                    bytes.fromhex(stylized_hypergraph["node_stroke_color"][1:])
                ),
                batch=batch,
            )
    if draw_hyperedges:
        ...
        for hyperedge in stylized_hypergraph["hyperedges"]:
            r = hyperedge["radius"]
            rectangles[hyperedge["id"]] = shapes.Rectangle(
                node["x_canvas"] - r,
...
def visualize(
    ...
    if type(palette) == str:
        ...
        if "compartment_colors" not in cfg:
            cfg["compartment_colors"] = [
                cols.colors[this_color] for this_color in cols.palettes[palette]
            ]
    ...
    with_plot = cfg["show_curves"] and set(ignore_plot_compartments) != set(
        compartments
    )  # ensure that all curves are not ignored
    ...
    hypergraph_batch = get_hypergraph_batch(
        hypergraph,
        yoffset=plot_height,
