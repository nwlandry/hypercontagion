...
def discrete_SIS2(H, tau, gamma, transmission_function=threshold, initial_infecteds=None, recovery_weight=None, transmission_weight = None, rho=None, tmin=0, tmax=float('Inf'), dt=1.0, **args):
    ...
    while t <= tmax and I[-1] != 0:
        ...
        for node in H:
            if status[node] == "I":
                ...
                for edge in H.nodes[node]["members"]:
                    if tau[len(H.edges[edge]["members"])] > 0:
                        if random.random() <= tau[len(H.edges[edge]["members"])]*transmission_function(status, H.edges[edge]["members"].difference({node}), **args)*dt*edgeweight(edge):
                            newStatus[node] = "I"
                            S[-1] += -1
                            I[-1] += 1
                            break
